...
async function plugin(fastify, options) {
    fastify.get("/turns", async (req, res) => {
        ...
        try {
            ...
            if (binSize < 1)
                binSize = 1;
            const { rows: data } = await fastify.pg.query(`SELECT FLOOR(turns/${binSize})*${binSize}::INT AS bin_floor, count(*)::INT FROM game WHERE turns > 10 AND DATE(timestamp) != CURRENT_DATE AND turns < 5 * (SELECT AVG(turns) FROM game WHERE turns > 10 AND DATE(timestamp) != CURRENT_DATE) GROUP BY 1 ORDER BY 1;`);
